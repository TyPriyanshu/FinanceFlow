...
export default function Dashboard() {
  ...
  const fetchStats = async () => {
    try {
      const data = await api.records.getStats();
      setStats(data);
    } catch (err) {
      console.error(err);
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          ...
          <h3 className="text-3xl font-bold text-gray-900">${(stats?.netBalance ?? 0).toLocaleString()}</h3>
        ...
        >
          ...
          <h3 className="text-3xl font-bold text-gray-900">${(stats?.totalIncome ?? 0).toLocaleString()}</h3>
        ...
        >
          ...
          <h3 className="text-3xl font-bold text-gray-900">${(stats?.totalExpense ?? 0).toLocaleString()}</h3>
